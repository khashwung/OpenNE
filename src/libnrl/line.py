...
class _LINE(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=1000, negative_ratio=5, order=3):
        ...
        # 种子需要传入initializer的种子中去，才能够起到固定随机化的作用
    ...
    def build_graph(self):
        ...
        cur_seed = random.getrandbits(32)
        # 一阶embedding和二阶上下文embedding，这是最终的训练结果
    ...
    def batch_iter(self):
        ...
        # (src1,dst1)和(dst1,src1)经过下面的方法可以唯一锁定到一条边
        ...
        while start_index < data_size:
            if mod == 0:
                ...
                for i in range(start_index, end_index):
                    # alias draw进行采样，根据边的权重随机从所有的边中随机采样一条边
                ...
                # 采一次正样本，采五次负样本，因为mod_size设为6，negative ratio设为5
                sign = -1.
                t = []
                # 针对某个h，随机采样可能没有与t有连边的负点
                for i in range(len(h)):
                    t.append(self.sampling_table[random.randint(0, table_size-1)])

            # 输出结果是迭代器类型，通过随机采样边，得到点对及其sign信息
            yield h, t, [sign]
            mod += 1
            mod %= mod_size
            if mod == 0:
    ...
    def gen_sampling_table(self):
        ...
        for edge in self.g.G.edges():
            # 对于每个边，统计该边触发的边的权重和，其实就相当于求所有的节点的度数
            node_degree[look_up[edge[0]]] += self.g.G[edge[0]][edge[1]]["weight"]

        # 对于每个度数，求其0.75次幂并求和，得到一个归一化项，这个是仿照自然语言处理中negative sampling的做法！
        #
        ...
        # 图中总共有多少条边
        data_size = self.g.G.number_of_edges()
        # alias table和prob table用于multinomial分布的快速采样
        ...
        # 归一化后的边的权重，边权重*总边数/总边权重。事实上这不是真正的归一化，真正的归一化不会*总边数，这里为了方便
        # 后面快速multinomial采样而*总边数的
        norm_prob = [self.g.G[edge[0]][edge[1]]["weight"]*data_size/total_sum for edge in self.g.G.edges()]
        num_small_block = 0
        num_large_block = 0
        cur_small_block = 0
        cur_large_block = 0
        # alias setup第一步，>1和<1集合的建立
        for k in range(data_size-1, -1, -1):
            if norm_prob[k] < 1:
                ...
                num_large_block += 1
        # alias setup第二步，计算alias table和prob table
...
class LINE(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=1000, epoch=10, negative_ratio=5, order=3, label_file=None, clf_ratio=0.5, auto_save=True):
        ...
        self.vectors = {}
        if order == 3:
            # order为3，为分别训练一阶和二阶的模型，最后得到一阶和二阶的embedding
    ...
    def get_embeddings(self):
        ...
        self.vectors = {}
        if self.order == 3:
            # 如果order为3，获取一阶和二阶的embedding并concat
